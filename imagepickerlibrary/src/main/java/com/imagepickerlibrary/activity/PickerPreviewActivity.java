package com.imagepickerlibrary.activity;
...
import com.imagepickerlibrary.FileChooseInterceptor;
import com.imagepickerlibrary.PickerAction;
import com.imagepickerlibrary.R;
import com.imagepickerlibrary.SImagePicker;
import com.imagepickerlibrary.util.FileUtil;
import com.imagepickerlibrary.util.ImageUtil;
import com.imagepickerlibrary.util.LogUtils;
import com.imagepickerlibrary.util.SystemUtil;
import com.imagepickerlibrary.widget.CheckBox;
import com.imagepickerlibrary.widget.PickerBottomLayout;
import com.imagepickerlibrary.widget.PicturePreviewPageView;
import com.imagepickerlibrary.widget.PreviewViewPager;
import com.imagepickerlibrary.widget.subsamplingview.ImageSource;
import com.imagepickerlibrary.widget.subsamplingview.OnImageEventListener;
...
public class PickerPreviewActivity extends BasePickerActivity implements PickerAction {
    ...
    public static void startPicturePreviewFromPicker(Activity activityContext,
                                                     ...
                                                     int maxCount, int rowCount,
                                                     FileChooseInterceptor fileChooseInterceptor,
                                                     @StringRes int pickRes,
                                                     @StringRes int pickNumRes,
                                                     AnchorInfo anchorInfo,
                                                     int requestCode) {
        Intent intent = new Intent(activityContext, PickerPreviewActivity.class);
        DataSave.setValue(KEY_CURRENT_POSITION, currentPosition);
        DataSave.setValue(KEY_URIS, pictures);
        DataSave.setValue(KEY_SELECTED, selected);
        DataSave.setValue(KEY_SELECTED_ORIGINAL, selectOriginal);
        DataSave.setValue(KEY_MAX_COUNT, maxCount);
        DataSave.setValue(KEY_ROW_COUNT, rowCount);
        DataSave.setValue(KEY_FILE_CHOOSE_INTERCEPTOR, fileChooseInterceptor);
        DataSave.setValue(PARAM_CUSTOM_PICK_TEXT_RES, pickRes);
        DataSave.setValue(KEY_ANCHOR_INFO, anchorInfo);
        activityContext.startActivityForResult(intent, requestCode);
    }

    public static class DataSave {
        public static int KEY_CURRENT_POSITION;
        public static ArrayList<Uri> KEY_URIS;
        public static ArrayList<String> KEY_SELECTED;
        public static boolean KEY_SELECTED_ORIGINAL;
        public static int KEY_MAX_COUNT = 9;
        public static int KEY_ROW_COUNT = 4;
        public static FileChooseInterceptor KEY_FILE_CHOOSE_INTERCEPTOR;
        public static int PARAM_CUSTOM_PICK_TEXT_RES;
        public static AnchorInfo KEY_ANCHOR_INFO;

        public static void setValue(String key, Object value) {
            if (key.equals(PickerPreviewActivity.KEY_CURRENT_POSITION)) {
                KEY_CURRENT_POSITION = (int) value;
            } else if (key.equals(PickerPreviewActivity.KEY_URIS)) {
                KEY_URIS = (ArrayList<Uri>) value;
            } else if (key.equals(PickerPreviewActivity.KEY_SELECTED)) {
                KEY_SELECTED = (ArrayList<String>) value;
            } else if (key.equals(PickerPreviewActivity.KEY_SELECTED_ORIGINAL)) {
                KEY_SELECTED_ORIGINAL = (boolean) value;
            } else if (key.equals(PickerPreviewActivity.KEY_MAX_COUNT)) {
                KEY_MAX_COUNT = (int) value;
            } else if (key.equals(PickerPreviewActivity.KEY_ROW_COUNT)) {
                KEY_ROW_COUNT = (int) value;
            } else if (key.equals(PickerPreviewActivity.KEY_FILE_CHOOSE_INTERCEPTOR)) {
                KEY_FILE_CHOOSE_INTERCEPTOR = (FileChooseInterceptor) value;
            } else if (key.equals(PickerPreviewActivity.PARAM_CUSTOM_PICK_TEXT_RES)) {
                PARAM_CUSTOM_PICK_TEXT_RES = (int) value;
            } else if (key.equals(PickerPreviewActivity.KEY_ANCHOR_INFO)) {
                KEY_ANCHOR_INFO = (AnchorInfo) value;
            }
        }

        public static void clear() {
            KEY_CURRENT_POSITION = 0;
            KEY_URIS = null;
            KEY_SELECTED = null;
            KEY_SELECTED_ORIGINAL = false;
            KEY_MAX_COUNT = 9;
            KEY_ROW_COUNT = 4;
            KEY_FILE_CHOOSE_INTERCEPTOR = null;
            PARAM_CUSTOM_PICK_TEXT_RES = 0;
            KEY_ANCHOR_INFO = null;
        }
    ...
    private static void showMaxDialog(Context context, int max) {
        new AlertDialog.Builder(context)
                .setMessage(context.getResources().getString(R.string.imagepicker_error_maximun_nine_photos, max))
                .setPositiveButton(R.string.imagepicker_general_ok, new DialogInterface.OnClickListener() {
                    @Override
    ...
    private void initUI() {
        ...
        max = DataSave.KEY_MAX_COUNT;
        rowCount = DataSave.KEY_ROW_COUNT;
        fileChooseInterceptor = DataSave.KEY_FILE_CHOOSE_INTERCEPTOR;
        ArrayList<Uri> uriParam = DataSave.KEY_URIS;
        ArrayList<String> selectedParam = DataSave.KEY_SELECTED;
        selectOriginal = DataSave.KEY_SELECTED_ORIGINAL;
        initPosition = DataSave.KEY_CURRENT_POSITION;
